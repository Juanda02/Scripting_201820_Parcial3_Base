    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.gameObject.tag == "Player")
        {
            if (isFollowing)
            {
                ...
                StartPatrullar();
                render.material.color = baseColor;
                GameManager.Instance.NextTagged(id, other.gameObject.GetComponent<Actor>().id);
                other.gameObject.GetComponent<Player>().SetTagged();
            }
        }*/
    }
        ...
        if(distanceTarget<2f)
        {
            ...
            if (playerTransform.gameObject.tag == "Player")
            {
                if (isFollowing)
                {
                    ...
                    playerTransform.gameObject.GetComponent<Player>().SetTagged();
                }
            }
        }
    }
